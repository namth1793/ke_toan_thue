...
/* ─── Static design elements (not in DB) ────────────────────── */
const painPoints = [
  { icon: '⚠️', pain: 'Lo lắng bị phạt thuế', solution: 'Chúng tôi nộp đúng hạn 100%, cam kết không phát sinh phạt.' },
  { icon: '❓', pain: 'Không hiểu luật thuế', solution: 'Đội ngũ chuyên gia cập nhật luật liên tục, xử lý thay bạn.' },
  { icon: '💸', pain: 'Chi phí kế toán full-time cao', solution: 'Dịch vụ thuê ngoài tiết kiệm 70% so với tuyển dụng nội bộ.' },
  { icon: '⏱️', pain: 'Mất thời gian làm giấy tờ', solution: 'Bàn giao tài liệu 1 lần, chúng tôi lo toàn bộ hàng tháng.' },
];

const plans = [
  { name: 'Cơ Bản', monthly: 500000, desc: 'Hộ kinh doanh, doanh nghiệp mới thành lập', features: [{ text: 'Kê khai thuế GTGT', ok: true }, { text: 'Kê khai thuế TNCN', ok: true }, { text: 'Lệ phí môn bài', ok: true }, { text: 'Hỗ trợ qua Zalo/Email', ok: true }, { text: 'Báo cáo tài chính đầy đủ', ok: false }, { text: 'Quyết toán thuế năm', ok: false }], cta: 'Bắt đầu', highlight: false },
  { name: 'Chuyên Nghiệp', monthly: 1500000, desc: 'Doanh nghiệp nhỏ và vừa đang hoạt động', features: [{ text: 'Tất cả dịch vụ Cơ Bản', ok: true }, { text: 'Báo cáo tài chính tháng/quý', ok: true }, { text: 'Quyết toán thuế năm', ok: true }, { text: 'Tư vấn thuế không giới hạn', ok: true }, { text: 'Hỗ trợ khi thanh tra thuế', ok: true }, { text: 'Kế toán trưởng riêng', ok: false }], cta: 'Đăng ký ngay', highlight: true, badge: 'Phổ biến nhất' },
  { name: 'Doanh Nghiệp', monthly: 3000000, desc: 'Doanh nghiệp giao dịch nhiều, phức tạp', features: [{ text: 'Tất cả dịch vụ Chuyên Nghiệp', ok: true }, { text: 'Kế toán trưởng riêng', ok: true }, { text: 'Lập kế hoạch tài chính', ok: true }, { text: 'Hỗ trợ vay vốn ngân hàng', ok: true }, { text: 'Tư vấn M&A cơ bản', ok: true }, { text: 'Ưu tiên xử lý trong ngày', ok: true }], cta: 'Liên hệ tư vấn', highlight: false },
];
...
export default function Home() {
  ...
  const svcSection = content?.home?.services || {};
  const testiSection = content?.home?.testimonials || {};
  const faqSection = content?.home?.faq || {};
  const settings = content?.settings || {};

  const stats = hero.stats || [{ value: '500+', label: 'Doanh nghiệp' }, { value: '10+', label: 'Năm kinh nghiệm' }, { value: '100%', label: 'Đúng hạn nộp thuế' }, { value: '0đ', label: 'Phí phạt phát sinh' }];
  const trustSignals = hero.trustSignals || ['Không phạt thuế muộn', 'Bảo mật thông tin tuyệt đối', 'Tư vấn không giới hạn'];
  const services = svcSection.items || [];
  const testimonials = testiSection.items || [];
  const phone = settings.phone || '0977.457.676';
  const phoneRaw = phone.replace(/\./g, '');
  const zaloLink = settings.zalo || `https://zalo.me/${settings.zaloPhone || '0977457676'}`;
    ...
    <div className="pt-nav">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-primary-600 via-primary-700 to-indigo-700 text-white">
        <div className="absolute inset-0 pointer-events-none" aria-hidden>
          <div className="absolute -top-32 -right-32 w-[500px] h-[500px] bg-sky-400/35 rounded-full blur-3xl" />
          <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-indigo-400/30 rounded-full blur-3xl" />
          <div className="absolute top-1/2 left-1/3 w-72 h-72 bg-blue-300/20 rounded-full blur-2xl" />
          ...
          <div className="grid lg:grid-cols-2 gap-12 xl:gap-20 items-center">
            ...
            <div>
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
                ...
                <div className="flex -space-x-1.5">
                  {['a1', 'a2', 'a3'].map((s, i) => (
                    <img key={i} src={`https://picsum.photos/seed/${s}/24/24`} alt="" className="w-6 h-6 rounded-full border-2 border-white/40 object-cover" />
                  ))}
                </div>
                <span className="text-sm text-white/90">
                  <span className="font-bold text-white">{stats[0]?.value || '500+'}</span> doanh nghiệp tin dùng
                </span>
              ...
              <motion.p initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.12 }}
                className="text-lg text-blue-100 leading-relaxed mb-7 max-w-lg">
                {hero.subheadline || 'Đội ngũ kế toán 10+ năm kinh nghiệm xử lý thuế, sổ sách, báo cáo — đúng hạn, đúng luật, không lo phạt.'}
              </motion.p>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.22 }}
                ...
                <Link to={hero.ctaLink || '/lien-he'} className="btn-white text-base px-7 py-3.5 shadow-lg">
                  {hero.ctaText || 'Tư vấn miễn phí ngay'}
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.15 }}
              ...
              <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-black/40 ring-1 ring-white/10">
                <img src="https://images.unsplash.com/photo-1554224155-6726b3ff858f?w=700&q=80&auto=format&fit=crop" alt="Kế toán chuyên nghiệp" className="w-full h-72 object-cover" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="relative rounded-xl overflow-hidden shadow-lg ring-1 ring-white/10">
                  <img src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=400&q=80&auto=format&fit=crop" alt="Đội ngũ tư vấn" className="w-full h-36 object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent" />
                  <p className="absolute bottom-2 left-3 text-white text-xs font-semibold">Đội ngũ chuyên gia</p>
                </div>
                <div className="relative rounded-xl overflow-hidden shadow-lg ring-1 ring-white/10">
                  <img src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=400&q=80&auto=format&fit=crop" alt="Báo cáo tài chính" className="w-full h-36 object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent" />
      ...
      <section className="py-20 bg-gradient-to-br from-primary-700 via-primary-800 to-indigo-800 text-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {painPoints.map((p, i) => (
              <ScrollFade key={p.pain} delay={i * 0.1}>
                <div className="flex gap-4 bg-white/10 hover:bg-white/15 transition-colors border border-white/15 rounded-2xl p-5">
                  ...
                  <div>
                    ...
                    <p className="text-white text-sm leading-relaxed">
                      <span className="text-emerald-400 font-semibold">→ </span>{p.solution}
                    </p>
                  </div>
                </div>
              </ScrollFade>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <ScrollFade className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-slate-800 mb-4">
                {svcSection.title || 'Mọi nhu cầu kế toán — '}<span className="text-gradient">{svcSection.title ? '' : 'một đối tác'}</span>
              </h2>
              {svcSection.subtitle && <p className="text-slate-500 max-w-xl mx-auto">{svcSection.subtitle}</p>}
            </ScrollFade>
              ...
              {services.map((s, i) => (
                <ScrollFade key={s.title} delay={i * 0.08}>
                  <div className="group bg-white border border-slate-100 rounded-2xl p-6 hover:border-primary-200 hover:shadow-lg hover:shadow-primary-50 transition-all duration-300 relative overflow-hidden">
                    ...
                    <div className="w-12 h-12 bg-primary-50 rounded-xl flex items-center justify-center mb-4 text-2xl">
                      {s.icon}
                    </div>
            ...
            <ScrollFade className="text-center mt-8">
              <Link to="/dich-vu" className="btn-secondary">Xem đầy đủ dịch vụ và bảng giá</Link>
            </ScrollFade>
          </div>
        </section>
      )}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <ScrollFade className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-slate-800 mb-3">
                {testiSection.title || 'Được tin dùng bởi hàng trăm doanh nghiệp Việt'}
              </h2>
              {testiSection.subtitle && <p className="text-slate-500 max-w-lg mx-auto">{testiSection.subtitle}</p>}
            </ScrollFade>
                  ...
                  <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-center gap-3 pt-4 border-t border-slate-50">
                      {t.avatar ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center text-primary-700 font-bold">{t.name?.[0]}</div>
                      )}
      ...
      <section className="py-20 bg-indigo-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollFade className="text-center mb-14">
            ...
            <p className="text-slate-500 max-w-lg mx-auto">Hợp đồng cố định 1 năm, giá không đổi. Nâng hoặc hạ gói bất cứ lúc nào.</p>
          </ScrollFade>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto items-start">
            {plans.map((plan, i) => (
              <ScrollFade key={plan.name} delay={i * 0.1}>
                <div className={`relative rounded-2xl overflow-hidden ${plan.highlight ? 'ring-2 ring-primary-500 shadow-2xl shadow-primary-100 scale-[1.02]' : 'bg-white border border-slate-100 shadow-sm'}`}>
                  {plan.highlight && (
                      ...
                      {plan.features.map((f) => (
                        <li key={f.text} className="flex items-center gap-2.5 text-sm">
                          {f.ok
                            ...
                            : <svg className="w-4 h-4 text-slate-200 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                          }
                          <span className={f.ok ? 'text-slate-700' : 'text-slate-300'}>{f.text}</span>
                        </li>
                      ))}
                    </ul>
                    <Link to="/lien-he" className={`block text-center py-3 rounded-xl font-semibold text-sm transition-all ${plan.highlight ? 'bg-gradient-to-r from-primary-600 to-indigo-600 text-white hover:opacity-90 shadow-lg shadow-primary-200' : 'bg-slate-900 text-white hover:bg-slate-700'}`}>
                      {plan.cta}
                    </Link>
                  </div>
                </div>
      ...
      {/* ══ CTA BANNER ════════════════════════════════════════════ */}
      <section className="relative py-20 overflow-hidden bg-gradient-to-br from-primary-600 via-indigo-600 to-primary-700 text-white">
        <div className="absolute inset-0 opacity-25" aria-hidden>
          <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-sky-300 rounded-full blur-3xl" />
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <ScrollFade>
            ...
            <h2 className="text-3xl md:text-5xl font-extrabold mb-4 leading-tight">Sẵn sàng để ngừng lo về thuế?</h2>
            <p className="text-blue-100 text-lg mb-8 max-w-2xl mx-auto">
              Hơn 500 doanh nghiệp đã chọn SORATA để yên tâm kinh doanh. Đăng ký hôm nay, bắt đầu ngay trong tuần.
            </p>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollFade className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-slate-800 mb-3">
              {faqSection.title || 'Câu hỏi thường gặp'}
            </h2>
